...
/**
 * Miro publishes its jobs on an Ashby board (descriptions inline), so the whole
 * portal is a single request. Miro is Amsterdam-headquartered with European
 * engineering hubs, so keep technical student-level titles in Europe and let
 * the LLM judge. 0 EU intern jobs is the normal steady state.
 */
export class MiroSource extends AshbySource {
  protected readonly defaultBoard = 'miro';
  protected readonly companyName = 'Miro';
