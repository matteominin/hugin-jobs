...
/**
 * Ramp publishes its jobs on an Ashby board (descriptions inline), so the whole
 * portal is a single request. Ramp is New-York-centric but runs a London
 * engineering office, so keep technical student-level titles in Europe and let
 * the LLM judge. 0 EU intern jobs is the normal steady state.
 */
export class RampSource extends AshbySource {
  protected readonly defaultBoard = 'ramp';
  protected readonly companyName = 'Ramp';
