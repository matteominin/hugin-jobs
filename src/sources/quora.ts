...
/**
 * Quora (which runs the Poe team) publishes on an Ashby board, so the whole
 * portal is a single request with descriptions inline. Quora is remote-first
 * and US-centric, so European-eligible roles surface via unknown/remote
 * locations; keep technical student-level titles and let the LLM judge. 0 EU
 * intern jobs is the normal steady state.
 */
export class QuoraSource extends AshbySource {
  protected readonly defaultBoard = 'quora';
  protected readonly companyName = 'Quora';
